...
export const linhas = pgTable('linhas', {
  ...
  horarioAberturaVotacao: varchar('horario_abertura_votacao', { length: 5 }).notNull().default('06:00'),
  /** Horário que finaliza votação para marcar lugares (HH:mm) */
  horarioFechamentoVotacao: varchar('horario_fechamento_votacao', { length: 5 }).notNull().default('07:30'),
...
export const onibus = pgTable('onibus', {
  id: uuid('id').primaryKey().defaultRandom(),
  idPrefeitura: uuid('id_prefeitura')
    .notNull()
    .references(() => prefeituras.id, { onDelete: 'cascade' }),
  /** Motorista que cadastrou o ônibus */
  idMotorista: uuid('id_motorista').references(() => usuarios.id, { onDelete: 'set null' }),
  numeroIdentificacao: varchar('numero_identificacao', { length: 20 }).notNull(),
  ...
  unique('uq_onibus_numero_prefeitura').on(table.idPrefeitura, table.numeroIdentificacao),
