...
describe('TripsService', () => {
  ...
  describe('scheduleTrips', () => {
    it('should schedule both OUTBOUND and INBOUND trips and generate IDs with suffixes and resolve shift from route departureTimeOutbound', async () => {
      const routeMock = {
        id: 'route-1',
        ...
        departureTimeOutbound: '08:30', // 08:30 is MORNING
      };
      ...
      const result = await service.scheduleTrips(
        {
          ...
          busId: 'bus-1',
          dates: ['2026-06-17'],
          shift: 'NIGHT', // Even if DTO says NIGHT, it should resolve to MORNING due to departureTimeOutbound '08:30'
          direction: 'OUTBOUND',
          realCapacity: 40,
        },
        'mun-1',
      ...
      expect(capturedValues[0].id).toBe('20260617-10155-M-O');
      expect(capturedValues[0].shift).toBe('MORNING');
      expect(capturedValues[1].id).toBe('20260617-10155-M-I');
      expect(capturedValues[1].shift).toBe('MORNING');
    });
    ...
    it('should fallback to dto.shift if route.departureTimeOutbound is not provided', async () => {
      const routeMock = {
        id: 'route-1',
        municipalityId: 'mun-1',
        weekDays: [1, 2, 3, 4, 5],
