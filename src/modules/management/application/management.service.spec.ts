...
describe('ManagementService', () => {
  ...
  describe('create', () => {
    it('should create a municipality', async () => {
      const insertChain = {
        ...
        returning: jest.fn().mockResolvedValue([{ id: 'mun1', nome: 'New City' }])
      };
      db.insert.mockReturnValue(insertChain as any);

      const result = await service.create({ name: 'New City' });
      expect(result.id).toBe('mun1');
      expect(insertChain.values).toHaveBeenCalledWith({ nome: 'New City' });
    });
  });
  ...
  describe('update', () => {
    ...
    it('should update successfully', async () => {
      const updateChain = {
        ...
        returning: jest.fn().mockResolvedValue([{ id: 'mun1', ativo: false }])
      };
      db.update.mockReturnValue(updateChain as any);

      const result = await service.update('mun1', { active: false });
      expect(result.ativo).toBe(false);
      expect(updateChain.set).toHaveBeenCalledWith({ ativo: false });
    });
  });
  ...
  describe('createManager', () => {
    ...
    it('should throw ConflictException if municipality already has manager', async () => {
      const mockChain = {
        from: jest.fn().mockReturnThis(),
        where: jest.fn().mockResolvedValue([{ id: 'mun1', idGestor: 'g1' }])
      };
      db.select.mockReturnValue(mockChain as any);
    ...
    it('should create manager successfully', async () => {
      ...
      const insertChain = {
        values: jest.fn().mockReturnThis(),
        returning: jest.fn().mockResolvedValue([{ id: 'user1', email: 'm@a.com' }])
      };
      db.insert.mockReturnValue(insertChain as any);
      ...
      const updateChain = {
        set: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        returning: jest.fn().mockResolvedValue([{ id: 'mun1', idGestor: 'user1' }])
      };
      db.update.mockReturnValue(updateChain as any);
      ...
      expect(updateChain.set).toHaveBeenCalledWith({ idGestor: 'user1' });
    });
  });
});
